...
export default function CalendarView({ trades, displayCurrency = "USD", isFree = false }: { trades: Trade[], displayCurrency?: string, isFree?: boolean }) {
  ...
  const renderTable = (data: PeriodStats[], primaryLabel: string) => {
    ...
    return (
      <div className="mt-8 bg-[#111827] border border-black/10 dark:border-[#111827] fade-slide-up shadow-[0_1px_2px_rgba(0,0,0,0.05)] dark:shadow-none rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="text-xs uppercase bg-[#1F2937] text-[#9CA3AF] border-b border-[#111827]">
              <tr>
            ...
            <tbody className="divide-y divide-white/5">
              {data.map((row, i) => (
                <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                  <td className="px-5 py-3 font-medium text-[#E5E7EB]">{row.label}</td>
                  <td className="px-5 py-3 text-[#E5E7EB]">{row.count}</td>
                  <td className="px-5 py-3 text-[#E5E7EB]">{row.count > 0 ? row.winRate.toFixed(1) + "%" : "-"}</td>
                  ...
                  <td className={cn("px-5 py-3 font-bold", row.pnl > 0 ? "text-emerald-400" : row.pnl < 0 ? "text-red-400" : "text-[#9CA3AF]")}>
              ...
              {/* Average Summary Row */}
              <tr className="bg-zinc-800/30 text-[#9CA3AF] italic">
              ...
              {/* Total Summary Row */}
              <tr className="bg-[#C9A646]/5 border-t-2 border-[#C9A646]/20 font-bold text-[#E5E7EB] shadow-inner">
                <td className="px-5 py-4 text-[#C9A646]">Total</td>
  ...
  return (
      <div className="space-y-2 animate-in fade-in duration-500 pb-10">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6 relative z-20 bg-[#111827] border border-black/10 dark:border-[#111827] fade-slide-up shadow-[0_1px_2px_rgba(0,0,0,0.05)] dark:shadow-none p-4 rounded-xl">
           <div className="flex items-center gap-4">
             <div className="flex bg-[#1F2937] p-1 rounded-lg border border-[#111827] shadow-inner">
               <button 
                 onClick={() => setViewMode("monthly")}
                  className={cn("px-4 py-1.5 text-xs font-bold rounded-md flex items-center gap-2 transition-all", effectiveViewMode === "monthly" ? "bg-zinc-800 text-[#E5E7EB] shadow-sm ring-1 ring-white/10" : "text-[#9CA3AF] hover:text-[#E5E7EB]")}
                >
                ...
                {!isFree && (
                  <button 
                    ...
                    className={cn("px-4 py-1.5 text-xs font-bold rounded-md flex items-center gap-2 transition-all", effectiveViewMode === "yearly" ? "bg-zinc-800 text-[#E5E7EB] shadow-sm ring-1 ring-white/10" : "text-[#9CA3AF] hover:text-[#E5E7EB]")}
                  >
           ...
           <div className="flex items-center gap-4">
              <button onClick={handlePrev} className="p-1.5 text-[#9CA3AF] hover:text-[#E5E7EB] hover:bg-zinc-800 rounded-lg transition-colors border border-transparent hover:border-[#111827]">
                <ChevronLeft size={20} />
              </button>
              <h2 className="text-lg font-black tracking-tight text-[#E5E7EB] w-32 text-center pointer-events-none">
                {effectiveViewMode === "monthly" ? format(currentDate, "MMMM yyyy") : format(currentDate, "yyyy")}
              </h2>
              <button onClick={handleNext} className="p-1.5 text-[#9CA3AF] hover:text-[#E5E7EB] hover:bg-zinc-800 rounded-lg transition-colors border border-transparent hover:border-[#111827]">
        ...
        {effectiveViewMode === "monthly" && (
          <div className="animate-in fade-in zoom-in duration-300">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              <div className="bg-[#111827] border border-black/10 dark:border-[#111827] fade-slide-up shadow-[0_1px_2px_rgba(0,0,0,0.05)] dark:shadow-none p-4 rounded-xl xl:col-span-2">
                <div className="grid grid-cols-7 gap-1">
                  {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
                    <div key={day} className="text-center text-xs font-bold text-[#9CA3AF] py-3 uppercase tracking-wider">{day}</div>
                  ))}
                  
                  {paddingDays.map((_, idx) => (
                    <div key={`padding-${idx}`} className="h-20 rounded-xl bg-[#1F2937]/30" />
                  ))}
                  ...
                  {daysInMonth.map((day) => {
                    ...
                    let bgColor = "bg-[#1F2937] hover:bg-[#111827]";
                    if (locked) {
                      bgColor = "bg-[#1F2937]/30 opacity-40 cursor-not-allowed";
                    } else if (hasTrades) {
                      bgColor = isProfit ? "bg-[#C9A646]/10 border-[#C9A646]/20 hover:bg-[#C9A646]/20" : "bg-red-500/10 border-red-500/20 hover:bg-red-500/20";
                    }
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          "h-20 flex flex-col items-center justify-center rounded-xl border border-black/10 dark:border-[#111827] shadow-[0_1px_2px_rgba(0,0,0,0.05)] dark:shadow-none transition-all relative",
                      ...
                      >
                        ...
                        <span className={cn("text-[13px] mb-1", isToday(day) ? "font-black text-emerald-400" : locked ? "text-zinc-600 font-bold" : "text-[#9CA3AF] font-bold")}>
                          {format(day, "d")}
                        </span>
                        {hasTrades && !locked && (
                          <span className={cn("text-[11px] font-bold px-1.5 py-0.5 rounded", isProfit ? "text-[#C9A646] bg-[#C9A646]/10" : "text-red-400 bg-red-500/10")}>
              ...
              <div className="bg-[#111827] border border-black/10 dark:border-[#111827] fade-slide-up shadow-[0_1px_2px_rgba(0,0,0,0.05)] dark:shadow-none p-5 flex flex-col rounded-xl h-[450px] xl:h-[500px] overflow-hidden">
                <div className="flex items-center justify-between mb-4 border-b border-[#111827] pb-4 shrink-0">
                  <h3 className="text-sm font-bold text-[#E5E7EB] flex items-center gap-2 tracking-wide">
                    {isFree ? "Day Review" : selectedDate ? format(selectedDate, "MMM do, yyyy") : "Day Review"}
                  </h3>
                  {selectedDate && !isFree && (
                    <button onClick={() => setSelectedDate(null)} className="p-1.5 bg-zinc-800 text-[#9CA3AF] hover:text-[#E5E7EB] rounded-lg transition-colors">
                      <X size={14} />
                    </button>
                  )}
                </div>
                ...
                <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
                  {!selectedDate ? (
                    <div className="h-full flex items-center justify-center text-[#9CA3AF] text-sm font-medium italic">
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center gap-3 py-8">
                      ...
                      <p className="text-[#9CA3AF] text-sm font-medium">This date is outside your trial period.</p>
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center">
                       ...
                       <p className="text-[#9CA3AF] text-sm font-medium">No recorded trades.</p>
                    </div>
                  ) : (
                    selectedTrades.map((trade, idx) => (
                      <div key={trade.id || idx} className="bg-[#1F2937] border border-black/10 dark:border-[#111827] shadow-[0_1px_2px_rgba(0,0,0,0.05)] dark:shadow-none p-4 rounded-xl flex flex-col gap-3 group hover:border-[#111827] transition-colors">
                        <div className="flex justify-between items-start">
                          <div>
                            <span className="font-bold text-[#E5E7EB] text-base mr-2 tracking-tight">{trade.symbol}</span>
                          ...
                          <span className={cn("font-black text-sm", (trade.normalizedPnl || 0) >= 0 ? "text-[#C9A646]" : "text-red-400")}>
                            {formatCurrency(Math.abs(trade.normalizedPnl || trade.pnl), displayCurrency)}
                          </span>
                        </div>
                        <div className="flex gap-4 text-xs font-semibold">
                          <span className="text-[#9CA3AF]">Vol: <span className="text-[#E5E7EB]">{trade.lot}</span></span>
                        </div>
                        {trade.note && (
                          <p className="text-xs text-[#9CA3AF] bg-white/5 p-2.5 rounded-lg italic">
                            &quot;{trade.note}&quot;
                          </p>
                        )}
                      </div>
        ...
        {effectiveViewMode === "yearly" && (
          <div className="animate-in fade-in zoom-in duration-300">
            ...
            <div className="bg-[#111827] border border-black/10 dark:border-[#111827] fade-slide-up shadow-[0_1px_2px_rgba(0,0,0,0.05)] dark:shadow-none p-5 rounded-xl mb-6">
               <h3 className="text-sm font-bold text-[#E5E7EB] mb-4 px-1 uppercase tracking-wider">Year-at-a-Glance Heatmap</h3>
                 ...
                 {monthlyStatsInYear.map((stat, i) => {
                   ...
                   return (
                     <div key={i} className={cn(
                       ...
                       !hasTrades ? "bg-[#1F2937]/50 border-[#111827]" :
                       isProfit ? "bg-[#C9A646]/5 border-[#C9A646]/20 hover:bg-[#C9A646]/10" : "bg-red-500/5 border-red-500/20 hover:bg-red-500/10"
                     )}>
                       <span className={cn("text-xs font-bold uppercase tracking-wider mb-2", hasTrades ? "text-[#E5E7EB]" : "text-[#9CA3AF]")}>
                         {stat.label.substring(0, 3)}
                       </span>
                       {hasTrades ? (
                         <>
                           <span className={cn("text-sm font-black text-center w-full truncate", isProfit ? "text-[#C9A646]" : "text-red-400")}>
                             {formatCurrency(Math.abs(stat.pnl), displayCurrency)}
                           </span>
                           <span className="text-[10px] text-[#9CA3AF] font-bold mt-1">{stat.count} Trades</span>
                         </>
                       ) : (
