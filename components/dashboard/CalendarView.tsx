...
export default React.memo(function CalendarView({ trades, displayCurrency = "USD", isFree = false }: { trades: Trade[], displayCurrency?: string, isFree?: boolean }) {
  ...
  const renderTable = (data: PeriodStats[], primaryLabel: string) => {
    ...
    return (
      <div className="mt-8 bg-gradient-to-b from-[#0A0A0A] to-[#121212] backdrop-blur-md border border-[rgba(212,175,55,0.15)] fade-slide-up shadow-[0_4px_24px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.02)] rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="text-xs uppercase bg-gradient-to-b from-[#0A0A0A] to-[#121212] backdrop-blur-md text-[#A0A0A0] border-b border-[#111827]">
              <tr>
            ...
            <tbody className="divide-y divide-white/5">
              {data.map((row, i) => (
                <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                  <td className="px-5 py-3 font-medium text-[#EAEAEA]">{row.label}</td>
                  <td className="px-5 py-3 text-[#EAEAEA]">{row.count}</td>
                  <td className="px-5 py-3 text-[#EAEAEA]">{row.count > 0 ? row.winRate.toFixed(1) + "%" : "-"}</td>
                  ...
                  <td className={cn("px-5 py-3 font-bold", row.pnl > 0 ? "text-emerald-400" : row.pnl < 0 ? "text-red-400" : "text-[#A0A0A0]")}>
              ...
              {/* Average Summary Row */}
              <tr className="bg-zinc-800/30 text-[#A0A0A0] italic">
              ...
              {/* Total Summary Row */}
              <tr className="bg-[#D4AF37]/5 border-t-2 border-[#D4AF37]/20 font-bold text-[#EAEAEA] shadow-inner">
  ...
  return (
      <div className="space-y-2 animate-in fade-in duration-500 pb-10">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6 relative z-20 bg-gradient-to-b from-[#0A0A0A] to-[#121212] backdrop-blur-md border border-[rgba(212,175,55,0.15)] fade-slide-up shadow-[0_4px_24px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.02)] p-4 rounded-2xl">
           <div className="flex items-center gap-4">
             <div className="flex bg-gradient-to-b from-[#0A0A0A] to-[#121212] backdrop-blur-md p-1 rounded-lg border border-[rgba(212,175,55,0.15)] shadow-inner">
               <button 
                 onClick={() => setViewMode("monthly")}
                  className={cn("px-4 py-1.5 text-xs font-bold rounded-md flex items-center gap-2 transition-all", effectiveViewMode === "monthly" ? "bg-zinc-800 text-[#EAEAEA] shadow-sm ring-1 ring-white/10" : "text-[#A0A0A0] hover:text-[#EAEAEA]")}
                >
                ...
                {!isFree && (
                  <button 
                    ...
                    className={cn("px-4 py-1.5 text-xs font-bold rounded-md flex items-center gap-2 transition-all", effectiveViewMode === "yearly" ? "bg-zinc-800 text-[#EAEAEA] shadow-sm ring-1 ring-white/10" : "text-[#A0A0A0] hover:text-[#EAEAEA]")}
                  >
           ...
           <div className="flex items-center gap-4">
              <button onClick={handlePrev} className="p-1.5 text-[#A0A0A0] hover:text-[#EAEAEA] hover:bg-zinc-800 rounded-lg transition-colors border border-transparent hover:border-[#111827]">
                <ChevronLeft size={20} />
              </button>
              <h2 className="text-lg font-black tracking-tight text-[#EAEAEA] w-32 text-center pointer-events-none">
                {effectiveViewMode === "monthly" ? format(currentDate, "MMMM yyyy") : format(currentDate, "yyyy")}
              </h2>
              <button onClick={handleNext} className="p-1.5 text-[#A0A0A0] hover:text-[#EAEAEA] hover:bg-zinc-800 rounded-lg transition-colors border border-transparent hover:border-[#111827]">
        ...
        {effectiveViewMode === "monthly" && (
          <div className="animate-in fade-in zoom-in duration-300">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              <div className="bg-gradient-to-b from-[#0A0A0A] to-[#121212] backdrop-blur-md border border-[rgba(212,175,55,0.15)] fade-slide-up shadow-[0_4px_24px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.02)] p-4 rounded-2xl xl:col-span-2">
                <div className="grid grid-cols-7 gap-1">
                  {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
                    <div key={day} className="text-center text-xs font-bold text-[#A0A0A0] py-3 uppercase tracking-wider">{day}</div>
                  ))}
                  ...
                  {daysInMonth.map((day) => {
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          "h-20 flex flex-col items-center justify-center rounded-2xl border border-[rgba(212,175,55,0.15)] shadow-[0_4px_24px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.02)] transition-all relative",
                      ...
                      >
                        ...
                        <span className={cn("text-[13px] mb-1", isToday(day) ? "font-black text-emerald-400" : locked ? "text-zinc-600 font-bold" : "text-[#A0A0A0] font-bold")}>
                          {format(day, "d")}
                        </span>
                        {hasTrades && !locked && (
                          <span className={cn("text-[11px] font-bold px-1.5 py-0.5 rounded", isProfit ? "text-emerald-400 bg-emerald-500/10" : "text-red-400 bg-red-500/10")}>
              ...
              <div className="bg-gradient-to-b from-[#0A0A0A] to-[#121212] backdrop-blur-md border border-[rgba(212,175,55,0.15)] fade-slide-up shadow-[0_4px_24px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.02)] p-5 flex flex-col rounded-2xl h-[450px] xl:h-[500px] overflow-hidden">
                <div className="flex items-center justify-between mb-4 border-b border-[#111827] pb-4 shrink-0">
                  <h3 className="text-sm font-bold text-[#EAEAEA] flex items-center gap-2 tracking-wide">
                    {isFree ? "Day Review" : selectedDate ? format(selectedDate, "MMM do, yyyy") : "Day Review"}
                  </h3>
                  {selectedDate && !isFree && (
                    <button onClick={() => setSelectedDate(null)} className="p-1.5 bg-zinc-800 text-[#A0A0A0] hover:text-[#EAEAEA] rounded-lg transition-colors">
                      <X size={14} />
                    </button>
                  )}
                </div>
                ...
                <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
                  {!selectedDate ? (
                    <div className="h-full flex items-center justify-center text-[#A0A0A0] text-sm font-medium italic">
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center gap-3 py-8">
                      ...
                      <p className="text-[#A0A0A0] text-sm font-medium">This date is outside your trial period.</p>
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center">
                       ...
                       <p className="text-[#A0A0A0] text-sm font-medium">No recorded trades.</p>
                    </div>
                  ) : (
                    selectedTrades.map((trade, idx) => (
                      <div key={trade.id || idx} className="bg-gradient-to-b from-[#0A0A0A] to-[#121212] backdrop-blur-md border border-[rgba(212,175,55,0.15)] shadow-[0_4px_24px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.02)] p-4 rounded-2xl flex flex-col gap-3 group hover:-translate-y-[2px] hover:border-[rgba(212,175,55,0.25)] hover:shadow-[0_8px_32px_rgba(212,175,55,0.1),inset_0_1px_0_rgba(255,255,255,0.02)] transition-all duration-300">
                        <div className="flex justify-between items-start">
                          <div>
                            <span className="font-bold text-[#EAEAEA] text-base mr-2 tracking-tight">{trade.symbol}</span>
                          ...
                          <span className={cn("font-black text-sm", (trade.normalizedPnl || 0) >= 0 ? "text-emerald-400" : "text-red-400")}>
                            {formatCurrency(Math.abs(trade.normalizedPnl || trade.pnl), displayCurrency)}
                          </span>
                        </div>
                        <div className="flex gap-4 text-xs font-semibold">
                          <span className="text-[#A0A0A0]">Vol: <span className="text-[#EAEAEA]">{trade.lot}</span></span>
                        </div>
                        {trade.note && (
                          <p className="text-xs text-[#A0A0A0] bg-white/5 p-2.5 rounded-lg italic">
                            &quot;{trade.note}&quot;
                          </p>
                        )}
                      </div>
        ...
        {effectiveViewMode === "yearly" && (
          <div className="animate-in fade-in zoom-in duration-300">
            ...
            <div className="bg-gradient-to-b from-[#0A0A0A] to-[#121212] backdrop-blur-md border border-[rgba(212,175,55,0.15)] fade-slide-up shadow-[0_4px_24px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.02)] p-5 rounded-2xl mb-6">
               <h3 className="text-sm font-bold text-[#EAEAEA] mb-4 px-1 uppercase tracking-wider">Year-at-a-Glance Heatmap</h3>
                 ...
                 {monthlyStatsInYear.map((stat, i) => {
                   ...
                   return (
                     <div key={i} className={cn(
                       ...
                       {hasTrades ? (
                         <>
                           <span className={cn("text-sm font-black text-center w-full truncate", isProfit ? "text-emerald-400" : "text-red-400")}>
                             {formatCurrency(Math.abs(stat.pnl), displayCurrency)}
                           </span>
                           <span className="text-[10px] text-[#A0A0A0] font-bold mt-1">{stat.count} Trades</span>
                         </>
                       ) : (
