...
export default function BulkPreviewModal({ isOpen, onClose, onSuccess, extractedTrades }: BulkPreviewModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex flex-col items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-5xl bg-[#0B0F14] rounded-2xl border border-[#111827] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
        
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-[#111827] bg-[#111827]/50 shrink-0">
          <div>
            ...
            <p className="text-sm text-[#9CA3AF] mt-1">Review the AI-mapped trades below. Correct any false readings before saving.</p>
          </div>
          <button onClick={onClose} className="p-2 text-[#9CA3AF] hover:text-[#E5E7EB] rounded-lg hover:bg-white/5 transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-auto p-6 custom-scrollbar bg-black/20">
          <div className="min-w-[800px]">
            <div className="grid grid-cols-8 gap-2 mb-3 px-2 text-xs font-bold text-[#9CA3AF] uppercase tracking-widest pb-2 border-b border-[#111827]">
            ...
            <div className="space-y-3">
              {trades.map((trade, idx) => (
                <div key={idx} className="grid grid-cols-8 gap-2 items-center bg-[#111827]/50 border border-[#111827] p-2 rounded-xl group hover:border-emerald-500/30 transition-colors">
                  ...
                  <div className="col-span-1">
                    <input 
                      ...
                      className={`w-full bg-black/40 border rounded px-3 py-2 text-sm font-bold focus:outline-none focus:border-emerald-500 transition-colors ${isValidValue("symbol", trade.symbol) ? "border-transparent text-[#E5E7EB]" : "border-red-500/50 text-red-400 focus:border-red-500"}`}
                    />
                  </div>
                  ...
                  <div className="col-span-1">
                    <input 
                      ...
                      className="w-full bg-black/40 border border-transparent rounded px-3 py-2 text-sm text-[#E5E7EB] focus:outline-none focus:border-emerald-500 transition-colors"
                    />
                  </div>
                  ...
                  <div className="col-span-1">
                    <input 
                      ...
                      className={`w-full bg-black/40 border rounded px-3 py-2 text-sm text-[#E5E7EB] focus:outline-none transition-colors ${isValidValue("entry", trade.entry) ? "border-transparent focus:border-emerald-500" : "border-red-500/50 text-red-400 focus:border-red-500"}`}
                    />
                  </div>
                  ...
                  <div className="col-span-1">
                    <input 
                      ...
                      className={`w-full bg-black/40 border rounded px-3 py-2 text-sm text-[#E5E7EB] focus:outline-none transition-colors ${isValidValue("exit", trade.exit) ? "border-transparent focus:border-emerald-500" : "border-red-500/50 text-red-400 focus:border-red-500"}`}
                    />
                  </div>

                  <div className="col-span-1 relative">
                    <div className="absolute left-3 top-1/2 -translate-y-1/2 text-[#9CA3AF] text-sm">$</div>
                    <input 
                      type="number" 
                      step="any"
                      value={trade.pnl} 
                  ...
                  <div className="col-span-1">
                    <input 
                      ...
                      className="w-full bg-black/40 border border-transparent rounded px-2 py-2 text-xs text-[#9CA3AF] focus:outline-none focus:border-emerald-500 transition-colors"
                    />
                  </div>
                  ...
                  <div className="col-span-1 flex justify-end">
                    <button 
                      ...
                      className="p-2 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-[#E5E7EB] rounded-lg transition-colors border border-red-500/20"
                      title="Discard Row"
                    >
                      <Trash2 size={16} />
                    </button>
        ...
        {/* Footer */}
        <div className="p-6 border-t border-[#111827] bg-[#111827]/50 shrink-0 flex items-center justify-between">
          <p className="text-xs text-[#9CA3AF] font-medium">* Commission variables are tracked but ignored for net PnL inputs by default.</p>
          <div className="flex gap-3">
            <button 
              onClick={onClose} 
              disabled={saving}
              className="px-6 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-[#E5E7EB] font-medium rounded-xl transition-colors border border-[#111827] disabled:opacity-50"
            >
              Cancel
            </button>
            <button 
              ...
              className="px-6 py-2.5 bg-[#C9A646] hover:bg-[#00e09d] text-zinc-950 font-bold rounded-xl transition-all shadow-md flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
