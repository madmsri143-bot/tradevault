...
export default function EditTradeModal({ trade, onClose }: { trade: Trade; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-[#111827] border border-[#111827] rounded-xl shadow-2xl w-full max-w-lg flex flex-col scale-100 animate-in zoom-in-95 duration-200 max-h-[80vh] overflow-hidden relative">
        
        <div className="flex items-center justify-between p-4 border-b border-[#111827] bg-[#111827] shrink-0 sticky top-0 z-10">
          <h2 className="text-lg font-semibold text-[#E5E7EB]">Edit Trade</h2>
          <button
            onClick={onClose}
            className="p-1.5 text-[#9CA3AF] hover:text-[#E5E7EB] hover:bg-zinc-800 rounded-md transition-colors"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col min-h-0">
          <div className="p-6 overflow-y-auto space-y-4 flex-1">
            ...
            {/* SymbolLine */}
            <div>
              <label className="block text-xs text-[#9CA3AF] mb-1">Symbol</label>
              <input
                type="text"
                name="symbol"
                ...
                className="w-full bg-[#1F2937] border border-zinc-800 rounded p-2 text-sm focus:border-emerald-500 focus:outline-none uppercase"
                required
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  value={formData.type}
                  onChange={handleChange}
                  className="w-full bg-[#1F2937] border border-zinc-800 rounded p-2 text-sm focus:border-emerald-500 focus:outline-none"
                >
              ...
              <div>
                <label className="block text-xs text-[#9CA3AF] mb-1">Lot Size</label>
                <input
                  type="number"
                  step="0.01"
                  name="lot"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-[#9CA3AF] mb-1">Result</label>
                <select
                  name="result"
                  value={formData.result}
                  onChange={handleChange}
                  className={`w-full bg-[#1F2937] border border-zinc-800 rounded p-2 text-sm focus:outline-none focus:border-emerald-500 font-medium ${formData.result === "Profit" ? "text-emerald-500" : "text-red-500"}`}
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#1F2937] border border-zinc-800 rounded p-2 text-sm focus:border-emerald-500 focus:outline-none"
                />
                <p className="text-[10px] text-[#9CA3AF] mt-1 leading-tight">Enter amount (profit or loss will be applied automatically)</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-[#9CA3AF] mb-1">Risk Amount</label>
                <input
                  type="number"
                  min="0"
                  step="0.01"
                  ...
                  className={`w-full bg-[#1F2937] border border-zinc-800 rounded p-2 text-sm focus:border-emerald-500 focus:outline-none ${!formData.stopLossFollowed ? 'opacity-40 cursor-not-allowed' : ''} ${riskAutoSynced ? 'border-amber-500/30' : ''}`}
                />
                {!formData.stopLossFollowed && (
                   <p className="text-[10px] text-[#9CA3AF] mt-1.5 font-medium">Risk not required when SL is not used</p>
                )}
              </div>
              <div>
                <label className="block text-xs text-[#9CA3AF] mb-1">SL Followed?</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-[#9CA3AF] mb-1">Currency</label>
                <select
                  name="currency"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  required
                  className="w-full bg-[#1F2937] border border-zinc-800 rounded p-2 text-sm focus:border-emerald-500 focus:outline-none color-scheme-dark"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-[#9CA3AF] mb-1">Entry Price (Optional)</label>
                <input
                  type="number"
                  step="any"
                  name="entryPrice"
              ...
              <div>
                <label className="block text-xs text-[#9CA3AF] mb-1">Exit Price (Optional)</label>
                <input
                  type="number"
                  step="any"
                  name="exitPrice"
            ...
            <div>
              ...
              <textarea
                ...
                onChange={handleChange}
                rows={3}
                className="w-full bg-[#1F2937] border border-zinc-800 rounded p-2 text-sm focus:border-emerald-500 focus:outline-none resize-none"
              />
            </div>
          </div>

          <div className="p-4 border-t border-[#111827] bg-[#111827] flex justify-end gap-3 shrink-0 sticky bottom-0 z-10">
            <button
              type="button"
              onClick={onClose}
              disabled={loading}
              className="px-4 py-2 text-sm text-[#9CA3AF] hover:text-[#E5E7EB] hover:bg-zinc-800 rounded font-medium transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="px-4 py-2 text-sm bg-emerald-600 hover:bg-emerald-500 text-[#E5E7EB] rounded flex items-center gap-2 font-medium transition-colors disabled:opacity-50"
            >
