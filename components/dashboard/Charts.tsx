  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#09090b] border border-[#27272a] rounded shadow-sm shadow-black/80 px-2 py-1.5 min-w-[50px] flex flex-col items-center">
        <p className="text-[11px] text-[#9CA3AF] font-medium tracking-tight mb-0.5 leading-none">{payload[0].payload.name}</p>
        <p className="text-[13.5px] font-bold text-[#E5E7EB] tracking-tight leading-none">
  ...
  return (
    <div className="flex flex-col gap-8 mb-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-[#111827] border border-black/10 dark:border-[#111827] fade-slide-up shadow-sm dark:shadow-md p-6 md:p-8 rounded-2xl flex flex-col items-center min-h-[360px] cursor-default hover:border-[#111827] transition-colors">
          <h3 className="text-[13px] font-bold tracking-wide uppercase text-[#9CA3AF] w-full mb-4">Win vs Loss</h3>
        {trades.length === 0 ? (
          <div className="flex-1 flex items-center justify-center text-[#9CA3AF] text-sm">No data</div>
        ) : (
        ...
        <div className="flex gap-4 text-xs text-[#9CA3AF] mt-2">
      ...
      <div className="bg-[#111827] border border-black/10 dark:border-[#111827] fade-slide-up shadow-sm dark:shadow-md p-6 md:p-8 rounded-2xl flex flex-col items-center justify-between min-h-[360px] cursor-default hover:border-[#111827] transition-colors">
        <h3 className="text-[13px] font-bold tracking-wide uppercase text-[#9CA3AF] w-full mb-6">Consistency Tracking</h3>
        
        {trades.length === 0 ? (
          <div className="flex-1 flex items-center justify-center text-[#9CA3AF] text-sm italic text-center px-4 leading-relaxed">
          ...
          <div className="w-full flex-1 flex flex-col justify-center items-center gap-6">
            <div className="flex flex-col items-center">
              <span className="text-xs text-[#9CA3AF] font-medium mb-1.5 uppercase tracking-widest">Current Streak</span>
              <div className={`text-4xl font-bold flex items-center gap-3 ${currentStreak > 0 ? "text-emerald-500 drop-shadow-[0_0_12px_rgba(16,185,129,0.3)]" : "text-[#9CA3AF]"}`}>
                <span>{currentStreak > 0 ? "🔥" : "❄️"}</span>
                {currentStreak} <span className="text-lg">Day{currentStreak !== 1 ? 's' : ''}</span>
              </div>
            </div>

            <div className="w-full max-w-[240px] bg-[#1F2937] p-4 rounded-xl border border-black/10 dark:border-[#111827] shadow-[0_1px_2px_rgba(0,0,0,0.05)] dark:shadow-none shadow-inner">
      ...
      <div className="bg-[#111827] border border-black/10 dark:border-[#111827] fade-slide-up shadow-sm dark:shadow-md p-6 md:p-8 rounded-2xl flex flex-col w-full relative z-10 transition-colors hover:border-[#111827]">
        ...
        {trades.length === 0 ? (
          <div className="flex-1 flex items-center justify-center text-[#9CA3AF] text-sm h-[250px] italic bg-[#1F2937]/50 rounded-lg border border-black/10 dark:border-[#111827] shadow-[0_1px_2px_rgba(0,0,0,0.05)] dark:shadow-none">
