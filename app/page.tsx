...
export default function Dashboard() {
  ...
  // Normalize Trades based on current exchange rates and selected displayCurrency
  const normalizedTrades = useMemo(() => {
    return trades.map((trade) => {
      ...
      if (Object.keys(rates).length > 0) {
        // Step 1: Convert trade to USD (Base)
        const tradeRate = rates[trade.currency] || 1;
        const amountInUSD = trade.pnl / tradeRate;

        // Step 2: Convert USD to Target Display Currency
  ...
  // Removed blocking loading spinner to make page open instantly
