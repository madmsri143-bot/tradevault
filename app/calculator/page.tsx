  ...
  return (
    <div className="relative group">
      <input 
        ...
        className="peer w-full bg-[#0B0F14]/50 border border-[#111827] rounded-xl px-4 pt-6 pb-2 text-sm text-[#E5E7EB] focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500/50 transition-all hover:bg-white/5 shadow-inner" 
      />
      <label 
        htmlFor={`input-${label.replace(/[\s/()]+/g, '-')}`}
        className="absolute left-4 top-4 text-xs font-bold uppercase tracking-widest text-[#9CA3AF] transition-all duration-200 peer-focus:-translate-y-2.5 peer-focus:text-[10px] peer-focus:text-amber-400 peer-[:not(:placeholder-shown)]:-translate-y-2.5 peer-[:not(:placeholder-shown)]:text-[10px] pointer-events-none"
      >
        {label}
      </label>
    </div>
  ...
  return (
    <div className="relative group">
      <select 
        ...
        className="peer w-full bg-[#0B0F14]/50 border border-[#111827] rounded-xl px-4 pt-6 pb-2 text-sm text-[#E5E7EB] focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500/50 transition-all hover:bg-white/5 appearance-none shadow-inner" 
      >
        {options.map((opt: any) => (
          <option key={opt.value || opt} value={opt.value || opt} className="bg-[#111827]">{opt.label || opt}</option>
        ))}
      </select>
      <label 
        htmlFor={`select-${label.replace(/[\s/()]+/g, '-')}`}
        className="absolute left-4 top-4 text-[10px] -translate-y-2.5 font-bold uppercase tracking-widest text-[#9CA3AF] transition-all duration-200 peer-focus:text-amber-400 pointer-events-none"
      >
        {label}
      </label>
      <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-[#9CA3AF] group-hover:text-amber-400 transition-colors">
        <ChevronDown size={14} />
      </div>
    </div>
  );
...
export default function CalculatorPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 max-w-5xl mx-auto pb-20 mt-4 px-4 sm:px-0">
      <div className="text-center sm:text-left flex flex-col sm:flex-row items-center gap-6 mb-8 border-b border-[#111827] pb-6 group">
        ...
        <div>
          <h1 className="text-4xl lg:text-5xl font-black tracking-tight text-[#E5E7EB] mb-2">Trading Calculators</h1>
          <p className="text-[#9CA3AF] font-medium text-lg">Manage risk, size positions, and calculate your edge.</p>
        </div>
      </div>
      ...
      <div className="relative bg-[#0B0F14]/60 backdrop-blur-2xl border border-[#111827] shadow-[0_10px_40px_rgba(0,0,0,0.5)] p-6 sm:p-10 rounded-3xl overflow-hidden min-h-[400px]">
        {/* Glow effects */}
    ...
    <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
      ...
      {result && !result.error && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="p-6 rounded-2xl bg-black/20 border border-[#111827] relative overflow-hidden group hover:border-[#111827] transition-colors">
            <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            <div className="relative z-10 text-xs font-black uppercase tracking-widest text-[#9CA3AF] mb-2">{getUnitName(instrument)} Output</div>
          ...
          <div className={`p-6 rounded-2xl border flex flex-col justify-center relative overflow-hidden group ${result.pnl >= 0 ? "bg-emerald-500/10 border-emerald-500/30" : "bg-red-500/10 border-red-500/30"}`}>
            ...
            <div className="relative z-10 text-xs font-black uppercase tracking-widest mb-2 opacity-60 text-[#E5E7EB]">{result.pnl >= 0 ? "Profit" : "Loss"}</div>
    ...
    <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
      ...
      {result !== null && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="p-6 rounded-2xl bg-black/20 border border-[#111827] relative overflow-hidden group hover:border-[#111827] transition-colors">
            <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            <div className="relative z-10 text-xs font-black uppercase tracking-widest text-[#9CA3AF] mb-2">Risk Amount</div>
            <div className="relative z-10 text-3xl font-black text-[#E5E7EB]">{formatMoney(result.riskAmount)}</div>
          </div>
    ...
    <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
      ...
      {result && !result.error && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="p-6 rounded-2xl bg-black/20 border border-[#111827] relative overflow-hidden group hover:border-[#111827] transition-colors">
            <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            <div className="relative z-10 text-xs font-black uppercase tracking-widest text-[#9CA3AF] mb-2">{unitName} Movement</div>
      ...
      <p className="text-xs text-[#9CA3AF] mt-6 text-center font-medium max-w-lg mx-auto">Measures the explicit absolute monetary value of the distance between two price points given your selected lot size/quantity.</p>
    </div>
  );
}
      ...
      {result && !result.error && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className={`p-5 rounded-2xl border flex flex-col justify-center relative overflow-hidden group md:col-span-2 lg:col-span-1 ${result.pnl >= 0 ? "bg-emerald-500/10 border-emerald-500/30" : "bg-red-500/10 border-red-500/30"}`}>
             <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
             <div className="relative z-10 text-[10px] font-black uppercase tracking-widest text-[#E5E7EB]/50 mb-1">Est PnL</div>
          ...
          <div className="p-5 rounded-2xl bg-black/20 border border-[#111827] relative overflow-hidden group hover:border-[#111827] transition-colors">
             <div className="relative z-10 text-[10px] font-black uppercase tracking-widest text-[#9CA3AF] mb-1">{getUnitName(instrument)} Gained</div>
          ...
          <div className="p-5 rounded-2xl bg-black/20 border border-[#111827] relative overflow-hidden group hover:border-[#111827] transition-colors">
             <div className="relative z-10 text-[10px] font-black uppercase tracking-widest text-[#9CA3AF] mb-1">Risk Amount</div>
             <div className="relative z-10 text-2xl font-black text-[#E5E7EB]">{formatMoney(result.riskAmount)}</div>
          </div>
          <div className="p-5 rounded-2xl bg-black/20 border border-[#111827] relative overflow-hidden group hover:border-[#111827] transition-colors">
             <div className="relative z-10 text-[10px] font-black uppercase tracking-widest text-[#9CA3AF] mb-1">Reward Ratio</div>
