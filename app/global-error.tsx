...
export default function GlobalError({
  ...
  return (
    <html lang="en">
      <body className="bg-black text-[#EAEAEA] flex items-center justify-center min-h-screen p-6 font-mono">
        <div className="max-w-2xl w-full bg-gradient-to-b from-[#0A0A0A] to-[#121212] backdrop-blur-md border border-red-500/50 p-8 rounded-2xl space-y-6">
          ...
          <div className="bg-black p-4 rounded-lg border border-[rgba(212,175,55,0.15)] overflow-x-auto">
            <p className="text-xl font-bold text-red-400 mb-2">{error.name || "Error"}</p>
            <p className="text-[#EAEAEA] whitespace-pre-wrap">{error.message}</p>
            {error.digest && <p className="text-[#A0A0A0] text-sm mt-4">Digest: {error.digest}</p>}
          </div>

          <div className="space-y-2">
            <h2 className="font-bold text-yellow-500">Diagnostic Check:</h2>
            <ul className="text-[#A0A0A0] text-sm space-y-1 list-disc pl-5 pb-4">
